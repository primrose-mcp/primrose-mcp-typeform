 ...
 * are passed via request headers, NOT stored in wrangler secrets. This allows a
 * single server instance to serve multiple customers.
 *
 * Request Headers:
 * - X-Typeform-Access-Token: Personal access token or OAuth token
 * - X-Typeform-Base-URL: (Optional) Override the default Typeform API base URL
 */
...
export interface TenantCredentials {
  /** Access Token for Typeform authentication (from X-Typeform-Access-Token header) */
  accessToken?: string;

  /** Override Typeform API base URL (from X-Typeform-Base-URL header) */
...
export function parseTenantCredentials(request: Request): TenantCredentials {
  ...
  return {
    accessToken: headers.get('X-Typeform-Access-Token') || undefined,
    baseUrl: headers.get('X-Typeform-Base-URL') || undefined,
  };
}
...
export function validateCredentials(credentials: TenantCredentials): void {
  if (!credentials.accessToken) {
    throw new Error('Missing credentials. Provide X-Typeform-Access-Token header.');
  }
}
